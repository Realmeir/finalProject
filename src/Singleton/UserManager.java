...
import Adapter.DictionaryAdapter;
import Adapter.Translator;
import Factory.*;
import ObserverAndStrategy.ExerciseObserver;
import ObserverAndStrategy.Exercises;
...
class UserManager {
    ...
    private String filePath = "user_data.txt";// File path for storing user data
    ...
    public UserManager() {
        ...
        exercises.registerObserver((ExerciseObserver) userProfile); // Register Singleton.UserProfile as an observer
    ...
    public void loginUser(UserProfile userProfile) { // Log in a user with the provided user profile
    ...
    private void useDictionary() {
        ...
        Translator translator = new DictionaryAdapter(dictionary);
        ...
        if (choice == 1) {
            ...
            String translation = translator.translateToRussian(inputWord);

            if (translation.equals("Перевод невозможен")) {
                System.out.println("Перевод не найден.");
            } else {
                System.out.println("Перевод: " + translation);
                System.out.println("Время перевода: " + translator.getLastTranslationTime() + " мс");
            }
        } else {
